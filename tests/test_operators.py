...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    assert (sigmoid(0 - abs(a)) <= 0.5 and sigmoid(1e9 + abs(a))) > 0.5
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    assert not bool(lt(a, b) and lt(b, c)) or bool(lt(a, c))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    assert all(
        math.isclose(a, b)
        for a, b in zip(addLists(ls1, ls2), [el1 + el2 for el1, el2 in zip(ls1, ls2)])
    )
